...
test('Config Test', async () => {
    let config = StudipGrantConfig.getGrantConfig(enabled, fakeStapi, provider, icon);
...
test('Config Test default icon', async () => {
    // @ts-ignore
    let config = StudipGrantConfig.getGrantConfig(enabled, fakeStapi, provider, null);
